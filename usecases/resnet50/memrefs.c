...
// struct memref_4d_f32

void alloc_memref_4d_f32(struct memref_4d_f32*s,
			 int size0, int size1, int size2, int size3) {
  int size = size0*size1*size2*size3*sizeof(float) ;
  // For some reason, aligned_alloc always returns NULL.
  // This is why I use an malloc-based approach.
  // s->allocated = aligned_alloc(sizeof(float),size) ;
  s->allocated = malloc(size+sizeof(float)) ;
...
void free_memref_4d_f32(struct memref_4d_f32*s) {
  free(s->allocated) ;
}

void print_memref_4d_f32(struct memref_4d_f32*s) {
}

void bzero_memref_4d_f32(struct memref_4d_f32*s) {
  bzero(s->aligned,s->size[0]*s->size[1]*s->size[2]*s->size[3]*sizeof(float)) ;
}
