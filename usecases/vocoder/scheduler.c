...
void sched_read_input_snd(int32_t      input_id,
  ...
  int r ;
  int datasize = sched_state.task_list[tid].inputs[input_id].datasize ;
  int shape = sched_state.task_list[tid].inputs[input_id].shape[0];
...
int32_t sched_write_output_snd(int32_t      output_id,
  ...
  // Write the data
  int w;
  int datasize = sched_state.task_list[tid].outputs[output_id].datasize ;
  int w_acc = datasize * sched_state.task_list[tid].outputs[output_id].shape[0];
  char* tmp_buf = (char*)aligned ;
