...
void task1_start(TASK_ID_TYPE tid) {
  ...
  resnet(tid, sched_read_input, sched_write_output);
}
...
void tick_task(TASK_ID_TYPE tid) {
  for(;;) {
    struct memref_4d_f32 to_task ;
    struct memref_2d_f32 from_task ;
    alloc_memref_4d_f32(&to_task,1,224, 224, 3) ;
    alloc_memref_2d_f32(&from_task,1,1000) ;
    ...
    sched_set_input_memrefxxxxf32(1,0,4,sizeof(float),
				  to_task.allocated,
				  to_task.aligned,
				  to_task.offset,
				  to_task.size[0],
